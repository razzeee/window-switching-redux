...
**App group target**:
A switcher target representing the windows outside the recent four belonging to
one eligible application. Selecting it emphasizes the group as a whole;
activating it focuses the group's newest surviving window.
_Avoid_: Older-window group, app icon

**Grouped window target**:
A switcher target representing one window within an app group target. Recent
windows never also appear as grouped window targets.
_Avoid_: Child item, nested window

**Eligible application**:
An application with at least one window outside the four recent windows. Its
app group target contains only those remaining windows, including when just
one remains.
...
an app group target, or a grouped window target. Each window belongs to either
an individual target or a group. A group target activates its newest member.
_Avoid_: Item, entry
...
**Edge context**:
The partially visible, inactive top-level targets at the left and right edges
while a group is entered. They preserve the surrounding grid's spatial context
but cannot receive selection or activation until the group is left.
...
same order, then eligible applications in the order of each group's newest
remaining window. Within an entered group, windows follow app-local
